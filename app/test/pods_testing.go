...
// ShowByTokenPodsInternalServerError runs the method ShowByToken of the given controller with the given parameters.
// It returns the response writer so it's possible to inspect the response headers and the media type struct written to the response.
// If ctx is nil then context.Background() is used.
// If service is nil then a default service is created.
func ShowByTokenPodsInternalServerError(t goatest.TInterface, ctx context.Context, service *goa.Service, ctrl app.PodsController, token string) (http.ResponseWriter, error) {
	// Setup service
	var (
		logBuf bytes.Buffer
		resp   interface{}
	...
	rw := httptest.NewRecorder()
	u := &url.URL{
		Path: fmt.Sprintf("/v1/pods/token/%v", token),
	}
	req, err := http.NewRequest("GET", u.String(), nil)
	if err != nil {
		panic("invalid test " + err.Error()) // bug
	...
	goaCtx := goa.NewContext(goa.WithAction(ctx, "PodsTest"), rw, req, prms)
	showByTokenCtx, _err := app.NewShowByTokenPodsContext(goaCtx, req, service)
	if _err != nil {
		e, ok := _err.(goa.ServiceError)
		if !ok {
			panic("invalid test data " + _err.Error()) // bug
...
// ShowByTokenPodsNotFound runs the method ShowByToken of the given controller with the given parameters.
// It returns the response writer so it's possible to inspect the response headers and the media type struct written to the response.
// If ctx is nil then context.Background() is used.
// If service is nil then a default service is created.
func ShowByTokenPodsNotFound(t goatest.TInterface, ctx context.Context, service *goa.Service, ctrl app.PodsController, token string) (http.ResponseWriter, error) {
	// Setup service
	var (
		logBuf bytes.Buffer
		resp   interface{}
...
// ShowByTokenPodsOK runs the method ShowByToken of the given controller with the given parameters.
// It returns the response writer so it's possible to inspect the response headers and the media type struct written to the response.
// If ctx is nil then context.Background() is used.
// If service is nil then a default service is created.
func ShowByTokenPodsOK(t goatest.TInterface, ctx context.Context, service *goa.Service, ctrl app.PodsController, token string) (http.ResponseWriter, *app.Pod) {
	// Setup service
	var (
		logBuf bytes.Buffer
		resp   interface{}
