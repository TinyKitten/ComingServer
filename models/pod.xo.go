...
func (p *Pod) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO comingserver.pods (` +
		`code, latitude, longitude, token, created_at, updated_at` +
		`) VALUES (` +
		`?, ?, ?, ?, ?, ?` +
		`)`

	// run query
	XOLog(sqlstr, p.Code, p.Latitude, p.Longitude, p.Token, p.CreatedAt, p.UpdatedAt)
	res, err := db.Exec(sqlstr, p.Code, p.Latitude, p.Longitude, p.Token, p.CreatedAt, p.UpdatedAt)
	if err != nil {
		return err
	}
...
func (p *Pod) Update(db XODB) error {
	...
	// run query
	XOLog(sqlstr, p.Code, p.Latitude, p.Longitude, p.Token, p.CreatedAt, p.UpdatedAt, p.ID)
	_, err = db.Exec(sqlstr, p.Code, p.Latitude, p.Longitude, p.Token, p.CreatedAt, p.UpdatedAt, p.ID)
	return err
}
...
func PodByCode(db XODB, code string) (*Pod, error) {
	...
	err = db.QueryRow(sqlstr, code).Scan(&p.ID, &p.Code, &p.Latitude, &p.Longitude, &p.Token, &p.CreatedAt, &p.UpdatedAt)
	if err != nil {
		return nil, err
	}
...
func PodByID(db XODB, id uint64) (*Pod, error) {
	...
	err = db.QueryRow(sqlstr, id).Scan(&p.ID, &p.Code, &p.Latitude, &p.Longitude, &p.Token, &p.CreatedAt, &p.UpdatedAt)
	if err != nil {
		return nil, err
	}
