...
func (p *Peer) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO comingserver.peers (` +
		`code, token, created_at, updated_at` +
		`) VALUES (` +
		`?, ?, ?, ?` +
		`)`

	// run query
	XOLog(sqlstr, p.Code, p.Token, p.CreatedAt, p.UpdatedAt)
	res, err := db.Exec(sqlstr, p.Code, p.Token, p.CreatedAt, p.UpdatedAt)
	if err != nil {
		return err
	}
...
func (p *Peer) Update(db XODB) error {
	...
	// run query
	XOLog(sqlstr, p.Code, p.Token, p.CreatedAt, p.UpdatedAt, p.ID)
	_, err = db.Exec(sqlstr, p.Code, p.Token, p.CreatedAt, p.UpdatedAt, p.ID)
	return err
}
...
func PeerByCode(db XODB, code string) (*Peer, error) {
	...
	err = db.QueryRow(sqlstr, code).Scan(&p.ID, &p.Code, &p.Token, &p.CreatedAt, &p.UpdatedAt)
	if err != nil {
		return nil, err
	}
...
func PeerByID(db XODB, id uint64) (*Peer, error) {
	...
	err = db.QueryRow(sqlstr, id).Scan(&p.ID, &p.Code, &p.Token, &p.CreatedAt, &p.UpdatedAt)
	if err != nil {
		return nil, err
	}
