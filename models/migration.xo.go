...
// Migration represents a row from 'comingserver.migrations'.
type Migration struct {
	ID        string         `json:"id"`         // id
	AppliedAt mysql.NullTime `json:"applied_at"` // applied_at
...
func (m *Migration) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO comingserver.migrations (` +
		`id, applied_at` +
		`) VALUES (` +
		`?, ?` +
		`)`
...
func (m *Migration) Update(db XODB) error {
	...
	// sql query
	const sqlstr = `UPDATE comingserver.migrations SET ` +
		`applied_at = ?` +
		` WHERE id = ?`
...
func (m *Migration) Delete(db XODB) error {
	...
	// sql query
	const sqlstr = `DELETE FROM comingserver.migrations WHERE id = ?`
...
// MigrationByID retrieves a row from 'comingserver.migrations' as a Migration.
//
