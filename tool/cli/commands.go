...
type (
	// ProfileAccountCommand is the command line data structure for the profile action of account
	ProfileAccountCommand struct {
		PrettyPrint bool
	}
	...
	// PeersListPodsCommand is the command line data structure for the peers list action of pods
	PeersListPodsCommand struct {
		// ポッドID
		ID int
		// 最大件数
		Limit int
...

{
   ...
   "screen_name": "tinykitten"
	...
	command = &cobra.Command{
		Use:   "peers-list",
		Short: `ポッドに属するすべてのピアを取得`,
	}
	tmp10 := new(PeersListPodsCommand)
	sub = &cobra.Command{
		Use:   `pods ["/v1/pods/ID/peers"]`,
		Short: ``,
	...
	tmp12 := new(ReceivePeerLocationWebsocketCommand)
	sub = &cobra.Command{
	...
	tmp13 := new(RegenerateTokenPeersCommand)
	sub = &cobra.Command{
	...
	command.AddCommand(sub)
	tmp14 := new(RegenerateTokenPodsCommand)
	sub = &cobra.Command{
	...
	tmp15 := new(SendCurrentPeerLocationWebsocketCommand)
	sub = &cobra.Command{
	...
	tmp16 := new(SendLocationPeersCommand)
	sub = &cobra.Command{
		Use:   `peers ["/v1/peers/ID/locations"]`,
		Short: ``,
		Long: `
...
// Run makes the HTTP request corresponding to the ProfileAccountCommand command.
func (cmd *ProfileAccountCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
	...
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.ProfileAccount(ctx, path)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
func (cmd *ProfileAccountCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
}
...
// Run makes the HTTP request corresponding to the PeersListPodsCommand command.
func (cmd *PeersListPodsCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
		path = fmt.Sprintf("/v1/pods/%v/peers", cmd.ID)
	}
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.PeersListPods(ctx, path, intFlagVal("limit", cmd.Limit), intFlagVal("offset", cmd.Offset))
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *PeersListPodsCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	var id int
